...
const kbDocs = ref<Array<{ id: number; title: string; score: number }>>([]);
...
async function send() {
  ...
  if (useKB.value) {
    try {
      const kbRes = await api.kbSearch(text);
  ...
  try {
    const resp = await api.chat(chatMessages, model.value);
    if (resp.ok) {
...
.kb-btn {
  ...
  height: 28px; padding: 0 12px; border: 1px solid var(--line);
...
.kb-btn--active { background: rgba(74,158,96,0.08); border-color: var(--success); color: var(--success); }
...
.kb-results__head {
  ...
  font-size: 11px; font-weight: 600; color: var(--ink-muted); margin-bottom: 6px;
}
.kb-result-item { display: flex; align-items: center; gap: 8px; padding: 3px 0; font-size: 12px; }
.kb-result-item__score {
  font-family: var(--font-mono); font-size: 10px; font-weight: 600;
  color: var(--success); width: 36px;
}
