...
export function useTranscription() {
  ...
  async function saveToLibrary() {
    ...
    if (!bridge || saved.value || saving.value) return;
    saving.value = true;
    try {
      const data = {
        ...saveContext.value,
        formattedText: resultText.value,
      };
      const resp = await bridge.saveRecord(data);
  ...
  async function runLink() {
    ...
    try {
      const response = await bridge.transcribeLink(raw);
      ...
      saveContext.value = (response as any).saveContext || { url: primaryUrl.value };
      saved.value = false;
      status.value = "完成。";
    } catch (error) {
      status.value = "失败。";
  ...
  async function runFile() {
    ...
    try {
      const response = await bridge.transcribeFile(path);
      logsText.value = response.logs.join("\n");
      resultText.value = response.text;
      videoUrl.value = response.videoUrl ?? "";
      stepTimings.value = (response.timings ?? []) as StepTiming[];
      saveContext.value = (response as any).saveContext || { url: localFileName.value };
      saved.value = false;
      status.value = "完成。";
    } catch (error) {
      status.value = "失败。";
